...
const checkApi = async (onSuccess) => {
    try {
        const response = await fetch(`/apps/optistep/get-popup?storeName=${storeName}`, {
        ...
        const rawResponse = await fetch(`/apps/optistep/get-popup?storeName=${storeName}`);
        const text = await rawResponse.text();
        console.error("❌ Raw HTML Response:\n", text);  // THIS will tell you what Shopify is actually sending back
    }
